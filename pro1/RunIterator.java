...
public class RunIterator implements Iterator {
  ...
  public boolean hasNext() {
    ...
    if(m_currentnode.getNext().getValue() != -1)
    {
        return true;
    }
    return false;
  ...
  public int[] next() {
    ...
    if(hasNext() == false)
    {
        throw new NoSuchElementException("there is no run anymore");
    }
    int[] runinfo = new int[4];
    m_currentnode = m_currentnode.getNext();
    runinfo[0] = m_currentnode.getRed();
    runinfo[1] = m_currentnode.getGreen();
    runinfo[2] = m_currentnode.getBlue();
    runinfo[3] = m_currentnode.getValue();
    return runinfo;
  }
