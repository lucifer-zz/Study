...
public class RunLengthEncoding implements Iterable {
   ...
   */
   ...
   // private DoubleLinkedList m_grayscaleimageinfo;
    private DoubleLinkedList m_runlengthsencoding;
  ...
  public RunLengthEncoding(int width, int height) {
    // Your solution here.
      ...
      m_runlengthsencoding = new DoubleLinkedList();
      newnode = new DNode(width * height, (short)0, (short)0, (short)0, null, null);
      m_runlengthsencoding.addFirst(newnode);
  ...
  public RunLengthEncoding(int width, int height, int[] red, int[] green,
                           int[] blue, int[] runLengths) {
    // Your solution here.
      int i, num = 0, size = red.length;
      if(!((size != 0)||(size == green.length) && (size == blue.length) && (size == runLengths.length)))
      {
          System.out.println("please check length");
          System.exit(0);
      }
      ...
      {
          num += runLengths[i];
          if((red[i] > 255)||(red[i] < 0)||(green[i] > 255)||(green[i] < 0)||(blue[i] > 255)||(blue[i] < 0))
          {
              System.out.println("Illeagal intensity input");
              System.exit(0);
          }
      }
      ...
      if(num != width*height)
      {
          System.out.println("Error: wrong image size");
          System.exit(0);
      }
      m_width = width;
      m_height = height;
      ...
      for(i = 0; i < size; i++)
      {
          newpixel = new DNode(runLengths[i], (short)red[i], (short)green[i], (short)blue[i], null, null);
          m_runlengthsencoding.addLast(newpixel);
      }
  ...
  public RunIterator iterator() {
    // Replace the following line with your solution.
    RunIterator iterator = new RunIterator(m_runlengthsencoding.getHeader());
    return iterator;
  ...
  public PixImage toPixImage() {
    // Replace the following line with your solution.
    PixImage curimage = new PixImage(m_width, m_height);

    RunIterator iter = iterator();
    int x, y, num = 0;
    int[] pixelinfo = iter.next();
    ...
    for(y = 0; y < m_height; y++)
    {
        for(x = 0; x < m_width; x++)
        {
            ...
            curimage.setPixel(x, y, (short)(pixelinfo[0]), (short)(pixelinfo[1]), (short)(pixelinfo[2]));
            num++;
        }
    }
    /*
  ...
  public String toString() {
    ...
    RunIterator iter = iterator();
    int[] pixelinfo = iter.next();
    String str = String.format("%d | %d | %d | %d", pixelinfo[0], pixelinfo[1], pixelinfo[2], pixelinfo[3]) + "\r\n";
    while(iter.hasNext())
    {
        pixelinfo = iter.next();
        str += String.format("%d | %d | %d | %d", pixelinfo[0], pixelinfo[1], pixelinfo[2], pixelinfo[3]) + "\r\n";
    }
    return str;
  }
  ...
  public RunLengthEncoding(PixImage image) {
    ...
    for(y = 0; y < m_height; y++)
    {
        for(x = 0; x < m_width; x++)
        {
            ...
            if(newnode != null)
            {
                if((newnode.getRed() == red)&&
                   (newnode.getGreen() == green)&&
                   (newnode.getBlue() == blue))
                {
                    runlength++;
                    continue;
                }
            ...
            if(newnode != null)
            {
                newnode.setValue(runlength);
            }

            newnode = new DNode(1, red, green, blue, null, null);
            m_runlengthsencoding.addLast(newnode);
            runlength = 1;
        }
    }
    check();
  ...
  public void check() {
    ...
    while(iter.hasNext())
    {
        ...
        if(runinfo[3] < 1)
        {
            errmsg = String.format("Error in run length: %d, red: %d, green: %d, blue: %d",
                                    runinfo[3], runinfo[0], runinfo[1], runinfo[2]);
            System.out.println(errmsg);
        }

        if(prevruninfo != null)
        {
            if((prevruninfo[0] == runinfo[0])&&(prevruninfo[1] == runinfo[1])&&(prevruninfo[2] == runinfo[2]))
            {
                errmsg = String.format("Error in run-length encoding: same run exit.") + "\r\n";
                errmsg += String.format("Prev Run Length: %d, Current Run Length: %d", prevruninfo[3], runinfo[3]) + "\r\n";
                errmsg += String.format("red: %d, green: %d, blue: %d", runinfo[0], runinfo[1], runinfo[2]);
                System.out.println(errmsg);
            }
        }
        prevruninfo = runinfo;
    ...
    if(m_width * m_height != sum)
    {
        errmsg = String.format("Error in runlengths: %d, should be %d", sum, m_width*m_height);
        System.out.println(errmsg);
    }
  ...
  public void setPixel(int x, int y, short red, short green, short blue) {
    ...
    //change
    if((red != curnode.getRed())||(green != curnode.getGreen())||(blue != curnode.getBlue()))
    {
        //adjust
        DNode positionnode, afterhalfnode;
        if(curnode.getValue() != 1)
        {
            ...
            else if(position == num -curnode.getValue())
            {
                if(curnode.getPrev() != m_runlengthsencoding.getHeader())
                {
                    ...
                    {
                        positionnode  = new DNode(1, red, green, blue, null, null);
                        m_runlengthsencoding.addBefore(curnode, positionnode);
                    }
                }
                else
                {
            ...
            else
            {
                ...
                {
                    if((curnode.getNext().getRed() == red)&&(curnode.getNext().getGreen() == green)&&(curnode.getNext().getBlue() == blue))
                    {
