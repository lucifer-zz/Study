...
public class Sorts {
   ...
   **/
    ...
    private static int mask = 0xf;
    private static int shiftstep = 4;
    private static int maxdigitfactor = 7;
  ...
  public static int[] countingSort(int[] keys, int whichDigit) {
    // Replace the following line with your solution.
      if(keys.length > 0)
      {
          ...
          {
              position = (keys[index] >> whichDigit * shiftstep) & mask;
              bucket[position]++;
          }
          ...
          {
              position = (keys[index] >> whichDigit * shiftstep) & mask;
              newarray[bucket[position] - 1] = keys[index];
              bucket[position]--;
          }
  ...
  public static int[] radixSort(int[] keys) {
    ...
    if(keys.length > 0)
    {
        int length = keys.length;
        int[] resultarr = keys;
        for(int i = 0; i <= maxdigitfactor; i++)
        {
            resultarr = countingSort(resultarr, i);
        }
